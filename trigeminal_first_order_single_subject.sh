...
usage() { echo "$(basename $0) [-s path_to_subject_dir] [-m path_to_mni_ROIs_clean] [-o output_dir] [-t nb_threads] -g true" 1>&2; exit 1; }
...
npv_first_order=10 # for quick testing only

#TODO: add a step size and theta parameter
#TODO: do a loop and perform ensemble tractography
...
nsub=${subject_dir}
nsub=`basename "$nsub"`
echo "Subject Folder: " ${subject_dir}
echo "Subject to process: " ${subject_dir}
...
# change made by Max: don't use quick version. Or add an optional argument in the script. But at first I would not make this the default. Too dangerous and against what I teach in my courses and papers. 
antsRegistrationSyN.sh \
    -d 3 -f ${nsub}/tractoflow/*__t1_warped.nii.gz \
...
## [ORIG-SPACE] Apply Transform to all ROIs from MNI space to orig_space of the patient
## These MNI space ROIs were manually drawn and defined by Samir with help from CIM
for nroi in cp_left cp_right
do
    antsApplyTransforms \
        -d 3 \
        -i ${mni_dir}/MNI/${nroi}.nii.gz \
        -r ${nsub}/tractoflow/*__t1_warped.nii.gz \
...
scil_volume_reslice_to_reference ${nsub}/freesurfer/aparc.DKTatlas+aseg.mgz \
 				 ${nsub}/tractoflow/${nsub}__t1_warped.nii.gz \
...
# WM mask
scil_volume_math lower_threshold ${nsub}/tractoflow/${nsub}__fa.nii.gz \
		 ${fa_threshold} \
...
do
    #TODO: here we add --step --theta options to mimic Joanisse Ensemble Tracking ISMRM 2021 abstract
    #TODO: should fix the --seed to have a reproducible script, no? @ARNAUD help.
    scil_tracking_local ${nsub}/tractoflow/${nsub}__fodf.nii.gz \
...
do
    ...
    #TODO: remove LOOPS?
    #TODO: remove strealines that are too short or too long? This could be based on the template and priors from the template
